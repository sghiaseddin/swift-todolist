...
jQuery(document).ready(function($) {
    ...
    $('.sh-todo-wrapper').on('click', '.create-task', function(e) {
        e.preventDefault();

        // Create a new task row at the beginning of the task list
        var newTaskRow = `
            <div class="sh-task-item new-task-item">
                <div class="sh-task-title">
                    <input type="text" class="new-task-title" placeholder="Enter task title">
                </div>
                <div class="sh-task-desc">
        ...
        `;
        $('.sh-todo-list-container').prepend(newTaskRow);
    });

    // Save New Task functionality
    $('.sh-todo-wrapper').on('click', '.save-task', function(e) {
        e.preventDefault();

        var newTaskItem = $(this).closest('.sh-task-item');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    newTaskItem.find('.sh-task-title').text(newTitle);
                    newTaskItem.find('.sh-task-desc').text(shortenDescription(newDesc)).data('full-desc', newDesc);
                    newTaskItem.find('.sh-task-actions').html(`
                        <a href="`+todolistBase+`/?action=view&id=`+response.data.task_id+`" title="View Task" class="view-task"><span class="dashicons dashicons-info-outline"></span></a>
    ...
    $('.sh-todo-wrapper').on('click', '.cancel-new-task', function(e) {
        e.preventDefault();

        // Remove the new task row
        $(this).closest('.sh-task-item').remove();
    });


    // Edit Task functionality with event delegation
    $('.sh-todo-wrapper').on('click', '.edit-task', function (e) {
        e.preventDefault();

        var taskItem = $(this).closest('.sh-task-item');
        var taskTitle = taskItem.find('.sh-task-title');
        var taskDesc = taskItem.find('.sh-task-desc');
        var actions = taskItem.find('.sh-task-actions');
    ...
    $('.sh-todo-wrapper').on('click', '.cancel-task', function (e) {
        e.preventDefault();
        ...
        if (fullDesc.length > 140) {
            taskItem.find('.sh-task-desc').addClass('long-desc'); // Class for styling :after "..."
            taskItem.find('.sh-task-desc').data('full-desc', fullDesc).text(shortenDescription(fullDesc));
        } else {
            taskItem.find('.sh-task-desc').data('full-desc', fullDesc).text(fullDesc);
        }
    ...
    $('.sh-todo-wrapper').on('click', '.update-task', function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success) {
                    ...
                    taskItem.find('.sh-task-title').text(newTitle);
                    if (newDesc.length > 140) {
                        taskItem.find('.sh-task-desc').addClass('long-desc'); // Class for styling :after "..."
                        taskItem.find('.sh-task-desc').data('full-desc', newDesc).text(shortenDescription(newDesc));
                    } else {
                        taskItem.find('.sh-task-desc').data('full-desc', newDesc).text(newDesc);
                    }

                    // Show other actions and hide cancel/update
                    taskItem.find('.edit-task, .remove-task, .view-task').show();
                    taskItem.find('.update-task, .cancel-task').hide();
                    newDesc = taskItem.find('.sh-task-desc').text();                            
    ...
    $('.sh-todo-wrapper').on('click', '.remove-task', function (e) {
        e.preventDefault();
